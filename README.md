...
| 💡 [Kafka Event Streaming Demo (Producer → Kafka → Consumer → PostgreSQL)](https://github.com/Nadia365/demoRepo-kafka) | Event-driven data pipeline using Kafka | This project demonstrates a simple event-driven architecture using Apache Kafka. A Python producer sends dataset events to a Kafka topic, a consumer processes those events, and the processed data is stored in PostgreSQL. The project illustrates core Kafka concepts such as producers, consumers, topics, offsets, and asynchronous communication between services. | Python / Apache Kafka / Docker / PostgreSQL |
